...
object Solver {
  ...
  abstract class Constraint(val variable: Variable, val variable2: Variable) {
    def revise(): Boolean = {
    ...
    override def equals(o: scala.Any): Boolean = {
      ...
      val other = o.asInstanceOf[Constraint]
      this.variable == other.variable && this.variable2 == other.variable2
    }
  }
  ...
  class RightToConstraint(variable: Variable, variable2: Variable) extends Constraint(variable, variable2) {
    override def update(first: Set[Int], second: Set[Int]): Set[Int] = {
      first.filter(value => second.exists(second => value > second))
    }
  }
  ...
  class ConstraintVariable(variable: Variable) {
    def same(variable2: Variable) = {
      List(new SameConstraint(variable, variable2), new SameConstraint(variable2, variable))
    }

    def leftTo(variable2: Variable) = {
      List(new LeftToConstraint(variable, variable2), new RightToConstraint(variable2, variable))
    }

    def nextTo(variable2: Variable) = {
      List(new NextToConstraint(variable, variable2), new NextToConstraint(variable2, variable))
    }

    def notSame(variable2: Variable) = {
      List(new NotSameConstraint(variable, variable2), new NotSameConstraint(variable2, variable))
    }
  }
  ...
  var constrains: List[Constraint] = first ++ second ++ third ++ fourth ++ fifth ++ sixth ++ eigth ++ tenth ++ eleventh ++ twelthe ++ thirteenth ++ fourteenth ++ fifteenth
  ...
  def solve(cv: Int = 0) = {
    var q = constrains.filter(constr => constr.variable2.index == cv && constr.variable.index > constr.variable2.index)
    var consistent = true
    while (q.nonEmpty && consistent) {
      val current = q.head
      q = q.tail
      if (current.revise()) {
        q = (q ++ constrains.filter { constraint =>
          current.variable2.index != constraint.variable.index && constraint.variable.index > cv
        }).distinct
        val k = current.variable
